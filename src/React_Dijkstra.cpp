...
class Graph {
    ...
    // Result array to store paths as strings
...
// Constructor: Initializes adjacency list and result array
Graph::Graph(int V) {
    this->V = V;
    adj = new list<iPair>[V];
    result = vector<string>(V, ""); // Initialize result array with empty strings
}
...
void Graph::shortestPath(int src) {
    ...
    // Save and print shortest paths from source to each vertex
    ...
    for (int i = 0; i < V; ++i) {
        ...
        if (dist[i] != INF) {
            ...
            string pathStr = ""; // To store the path as a string
            while (!path.empty()) {
                int node = path.top();
                path.pop();
                pathStr += to_string(node);
                if (!path.empty()) pathStr += "|";
            }

            result[i] = pathStr; // Save the path in the result array
            printf("%s", pathStr.c_str()); // Print the path
        }
        printf("\n");
    }
}
